...
The updated, property-first key is encoded in `matrix_key/structured_dichotomous_key.yaml` and produces a final estimate Type alongside downgrade flags. The same key is executed by the R classification engine and the Shiny app, ensuring one canonical implementation and a consistent interpretation of estimate-type guidance.
...
csas_kable(
  ...
  kableExtra::column_spec(1, width = "7.0cm") |>
...
csas_kable(
  ...
  kableExtra::column_spec(2, width = "7.2cm") |>
  kableExtra::column_spec(3, width = "3.8cm")
```
