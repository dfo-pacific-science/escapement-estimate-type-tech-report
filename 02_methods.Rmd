...
The classification key follows a property-first sequence that matches the current implementation in `matrix_key/structured_dichotomous_key.yaml`:
...
The canonical decision key is stored in `matrix_key/structured_dichotomous_key.yaml`. The classification engine in `R/classification_engine.R` loads the YAML and evaluates answers to produce the final type and flags. The Shiny app in `app.R` surfaces the questions and exports classification results for review. The comprehensive path tester in `R/comprehensive_path_testing.R` exercises representative paths across all method families and checks expected outcomes.
