...
A companion internal R Shiny application applies this decision logic and returns both (i) a final estimate Type (1--6) and (ii) qualifier codes explaining any conservative downgrades. The software is a companion tool; it is **not** the primary output of this report.
...
The table below lists guidance-level metadata elements and the typical qualifier codes they support; detailed software rollout and database-change logistics are treated as companion implementation work.
...
Implementation should proceed in stages so the guidance can be applied consistently before extending scope:

- prioritize SEN-only fields needed for transparent Type assignment and conservative downgrades;
- finalize controlled lookup tables for those fields so downstream implementation remains interoperable; and
- defer broader SIL/CU enhancements to a later governance cycle.

This keeps the technical report focused on interpretability while leaving software rollout details to companion implementation tracks.
...
Detailed term-level crosswalks are provided in Appendix B (downgrade criteria) and Appendix C (enumeration and estimate methods), and equivalent lookup helpers are also exposed in the `metasalmon` package (`nuseds_enumeration_method_crosswalk()` and `nuseds_estimate_method_crosswalk()`) [@metasalmonPackage].
