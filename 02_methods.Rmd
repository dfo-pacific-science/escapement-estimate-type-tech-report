...
The guidance is implemented in the **SMN Escapement Estimates Toolkit** (R Shiny application) [@smnToolkitShiny]. The toolkit executes one canonical decision key and produces (i) a final estimate Type (1--6) and (ii) explicit qualifier codes explaining conservative downgrades.
...
## Companion implementation status snapshot (current)

A focused implementation-status check was completed against the current toolkit codebase [@smnToolkitShiny] to confirm that report guidance and software behavior remain aligned.

Current snapshot highlights:

- The YAML decision key currently encodes **8 method families** (FS, V, A, S, T, R, P, M), **20 downgrade criteria**, and **6 invariant rules**.
- The comprehensive path-test suite currently reports **26/26 passing** scenarios across method families, gate behavior, and downgrade pathways.
- The app's "Known Issues" panel currently flags unresolved policy/specification items (e.g., mixed-method combination policy, quantifiable thresholds for ambiguous terms, and calibration diagnostic standards) that should be treated as active implementation backlog rather than closed guidance questions.
...
**Note:** This section distinguishes (i) guidance-level metadata elements and (ii) implementation-aligned NuSEDS fields currently prioritized for SEN uploads. The key point is that these elements are recorded by the guidance as explicit qualifier codes so Type assignment remains transparent to downstream data users.
...
## Implementation-aligned NuSEDS additions (SEN-first rollout)

Task-team correspondence and implementation notes indicate a near-term SEN-first rollout. The table below captures the specific fields and supporting lookup tables discussed for implementation.

```{r nuseds-additions-implementation-table}
nuseds_additions <- data.frame(
  `Field / table` = c(
    "ESTIMATE_TYPE_CLASS_JUSTIFICATION",
    "DOWNGRADE_CRITERIA",
    "UNCERTAINTY_VALUE_TYPE",
    "UNCERTAINTY_VALUE",
    "ANALYSIS_EXPLANATION",
    "DOWNGRADE_CRITERIA_BASE_TABLE",
    "UNCERTAINTY_VALUE_TYPE_BASE_TABLE",
    "DOWNGRADE_CRITERIA_COMMENT",
    "WSP_RAPID_STATUS_YN",
    "ENHANCEMENT_RANK / POP_CATEGORY / CUMatchVerifiedYN"
  ),
  `Scope` = c(
    "SEN upload",
    "SEN upload",
    ...
    "Lookup / controlled values",
    "Lookup table column",
    "SEN upload (candidate)",
    "Site/CU metadata (candidate)"
  ),
  `Role in guidance` = c(
    "Documents why a Type was assigned.",
    "Records explicit downgrade triggers used by the decision key.",
    "Captures uncertainty metric class (e.g., CV, SE, CI, SD).",
    "Stores numeric uncertainty value paired to uncertainty type.",
    "Documents analytical workflow, including modified/combined methods.",
    "Provides controlled downgrade criteria values for consistent entry.",
    "Provides controlled uncertainty metric values and definitions.",
    "Replaces prior IMPACT field with broader comment context.",
    "Flags rapid-status use; discussed but not fully settled.",
    "Supports WSP/CU interpretation context; discussed for follow-on integration."
  ),
  `Implementation status` = c(
    ...
    "Created in Dev",
    "Created in Dev",
    "Renamed from DOWNGRADE_CRITERIA_IMPACT",
    "Under discussion",
    "Under discussion"
  ),
  check.names = FALSE
...
csas_kable(
  ...
  font_size = 8,
  align = c("l", "l", "l", "l"),
  caption = "Implementation-aligned NuSEDS field additions discussed for estimate-type guidance rollout."
) |>
  csas_colspec(1, width = "3.8cm") |>
  csas_colspec(2, width = "2.8cm") |>
  csas_colspec(3, width = "6.0cm") |>
  csas_colspec(4, width = "2.4cm")
```

## Ontology linkages for methods and downgrade criteria

To support interoperable interpretation, the SEN implementation fields are linked to the published DFO Salmon Ontology concept schemes and term pages (WIDOCO documentation):

- ontology entrypoint: <https://w3id.org/gcdfo/salmon>
- WIDOCO documentation (latest): <https://dfo-pacific-science.github.io/dfo-salmon-ontology/>
...
if (file.exists(dd_path)) {
  ...
  dd_small[["Field Definition"]] <- ifelse(
    ...
    paste0(substr(dd_small[["Field Definition"]], 1, 139), "…"),
